...
const EditProfileScreenOwner = (props) => {
  ...
  const [numBreeds, setNumBreeds] = useState([]);
  const { currentUser } = props;
  const userUID = "";

  const addTextInput = (index) => {
    let breeds = breeds;
    this.state.breeds.push(
      <TextInput onChangeText={(text) => addValues(text, index)} />
    );
  ...
  const addValues = (text, index) => {
    ...
    if (checkBool) {
      setNumBreeds(dataArr);
    } else {
      dataArr.push(text);
    }
  };

  const handleInput = (breed) => {
    const _breeds = [...breeds];
    _breeds.push(breed);
    setBreeds(_breeds);
  };
      ...
      <View style={styles.container}>
        <View style={{ margin: 20 }}>
          ...
          <View>
            ...
            <Button title="Get Values" onPress={() => {}} />
          </View>
