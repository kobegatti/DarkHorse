...
class SignUpScreenOwner extends Component {
  ...
  handleSubmit() {
    ...
    if (this.state.password !== this.state.passwordConfirm) {
      ...
      let val = "Passwords do not match";
      return this.updateInputValue(val, "error");
    } else if (
      this.state.username === "" ||
      this.state.email === "" ||
      this.state.password === ""
      ...
      new_state["error"] = "Missing form entry";
      this.setState(new_state);
      let val = "Missing form entry";
      this.updateInputValue("Missing form entry", "error");
      console.log("error = " + this.error);
      return this.updateInputValue(val, this.error);
    } else if (this.state.username.length > MAX_ENTRY_LENGTH) {
      return this.updateInputValue(
        "Username must be 20 characters of less",
        "error"
      );
